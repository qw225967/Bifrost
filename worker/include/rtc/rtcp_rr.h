...
class ReceiverReport {
 ...
 public:
  ...
  ReceiverReport() { this->header = reinterpret_cast<Header*>(this->raw); }
  ...
  uint8_t GetFractionLost() const {
    return uint8_t{Byte::get_1_byte((uint8_t*)this->header, 4)};
  }
  void SetFractionLost(uint8_t fractionLost) {
    Byte::set_1_byte((uint8_t*)this->header, 4, fractionLost);
  }
  int32_t GetTotalLost() const {
    auto value = uint32_t{Byte::get_3_bytes((uint8_t*)this->header, 5)};

    // Possitive value.
    if (((value >> 23) & 1) == 0) return value;

    // Negative value.
    if (value != 0x0800000) value &= ~(1 << 23);
  ...
  void SetTotalLost(int32_t totalLost) {
    ...
    uint32_t value =
        (totalLost >= 0) ? (clamp & 0x07FFFFF) : (clamp | 0x0800000);

    Byte::set_3_bytes(reinterpret_cast<uint8_t*>(this->header), 5, value);
  }
  uint32_t GetLastSeq() const { return uint32_t{ntohl(this->header->lastSeq)}; }
  void SetLastSeq(uint32_t lastSeq) {
    this->header->lastSeq = uint32_t{htonl(lastSeq)};
  }
  uint32_t GetJitter() const { return uint32_t{ntohl(this->header->jitter)}; }
...
class ReceiverReportPacket : public RtcpPacket {
 ...
 public:
  ...
  uint32_t GetSsrc() const { return this->ssrc; }
  void SetSsrc(uint32_t ssrc) { this->ssrc = ssrc; }
  void AddReport(ReceiverReport* report) { this->reports.push_back(report); }
 ...
 public:
  ...
  // contains receive report blocks we also generate a second
  // ReceiverReportPacket from same data and len, so parent Packet::GetType()
  // would return this->type which would be SR instead of RR.
  Type GetType() const override { return Type::RR; }
  size_t GetCount() const override { return this->reports.size(); }
  size_t GetSize() const override {
    size_t size = sizeof(RtcpPacket::CommonHeader) + 4u /* this->ssrc */;
