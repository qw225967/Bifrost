...
# 配置两个端口容器的网络，并且暴露udp端口对外
docker run -d --cap-add=NET_ADMIN --network leftnet --ip 10.0.0.100 -e GATEWAY="10.0.0.2" -p 8889:8889/udp --name client endpoint
docker run -d --cap-add=NET_ADMIN --network rightnet --ip 10.100.0.100 -e GATEWAY="10.100.0.2"  -p 8887:8887/udp --name server endpoint
cd ..
