...
std::shared_ptr<FeedbackRtpItemsPacket<Item>> FeedbackRtpItemsPacket<Item>::Parse(const uint8_t* data, size_t len)
{
  if (len < sizeof(CommonHeader) + sizeof(FeedbackPacket::Header))
  {
    std::cout << "[feedback rtp items] not enough space for Feedback packet, discarded" << std::endl;
  ...
  std::shared_ptr<FeedbackRtpItemsPacket<Item>> packet = std::make_shared<FeedbackRtpItemsPacket<Item>>(commonHeader);
