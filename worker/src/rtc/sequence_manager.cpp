...
bool SeqManager<T>::Input(const T input, T& output) {
  ...
  if (!this->dropped.empty()) {
    ...
    if (it != this->dropped.end()) {
      ...
      if (*it == input) {
        std::cout << "[sequence manager] trying to send a dropped input"
                  << std::endl;
  ...
  // New input is higher than the maximum seen. But less than acceptable units
  // higher. Keep it as the maximum seen. See Drop().
  if (idelta < MaxValue / 2) this->maxInput = input;

  // New output is higher than the maximum seen. But less than acceptable units
  // higher. Keep it as the maximum seen. See Sync().
  if (odelta < MaxValue / 2) this->maxOutput = output;
