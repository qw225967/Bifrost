...
#define TODO_PRINT_PROBING_STATE()                                   \
  switch (probing_state_) {                                          \
    case ProbingState::kDisabled:                                    \
      MS_DUMP("--- probing_state_:kDisabled, clusters_.size():%zu",  \
              clusters_.size());                                     \
      break;                                                         \
    case ProbingState::kInactive:                                    \
      MS_DUMP("--- probing_state_:kInactive, clusters_.size():%zu",  \
              clusters_.size());                                     \
      break;                                                         \
    case ProbingState::kActive:                                      \
      MS_DUMP("--- probing_state_:kActive, clusters_.size():%zu",    \
              clusters_.size());                                     \
      break;                                                         \
    case ProbingState::kSuspended:                                   \
      MS_DUMP("--- probing_state_:kSuspended, clusters_.size():%zu", \
              clusters_.size());                                     \
      break;                                                         \
  }
...
int BitrateProber::TimeUntilNextProbe(int64_t now_ms) {
  ...
  if (probing_state_ != ProbingState::kActive || clusters_.empty()) return -1;
...
void BitrateProber::ProbeSent(int64_t now_ms, size_t bytes) {
  ...
  if (!clusters_.empty()) {
    ...
    if (clusters_.empty()) probing_state_ = ProbingState::kSuspended;
