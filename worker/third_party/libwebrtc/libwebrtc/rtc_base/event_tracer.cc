...
class EventLogger final {
 public:
  ...
  void AddTraceEvent(const char* name, const unsigned char* category_enabled,
                     char phase, int num_args, const char** arg_names,
                     const unsigned char* arg_types,
                     const unsigned long long* arg_values, uint64_t timestamp,
                     int pid, rtc::PlatformThreadId thread_id) {
...
void InternalAddTraceEvent(char phase, const unsigned char* category_enabled,
                           const char* name, unsigned long long id,
                           int num_args, const char** arg_names,
                           const unsigned char* arg_types,
                           const unsigned long long* arg_values,
  ...
  if (rtc::AtomicOps::AcquireLoad(&g_event_logging_active) == 0) return;
...
bool StartInternalCapture(const char* filename) {
  if (!g_event_logger) return false;
