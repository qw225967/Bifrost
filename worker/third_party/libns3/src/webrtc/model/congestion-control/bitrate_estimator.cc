...
BitrateEstimator::BitrateEstimator()
    ...
    uncertainty_symmetry_cap_(RtcDataRate::Zero()),
    estimate_floor_(RtcDataRate::Zero()),
...
void BitrateEstimator::Update(Timestamp at_time, DataSize amount, bool in_alr) {
    ...
    //std::cout << "ack bitrate = " << bitrate_estimate_kbps_ << "kbps!" << std::endl;
}
...
std::optional<RtcDataRate> BitrateEstimator::bitrate() const {
    if (bitrate_estimate_kbps_ < 0.f)
        return std::nullopt;
    return RtcDataRate::KilobitsPerSec(bitrate_estimate_kbps_);
}

std::optional<RtcDataRate> BitrateEstimator::PeekRate() const {
