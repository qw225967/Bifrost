...
double LossFromBitrate(RtcDataRate bitrate,
	RtcDataRate loss_bandwidth_balance,
	double exponent) {
...
RtcDataRate BitrateFromLoss(double loss,
	RtcDataRate loss_bandwidth_balance,
	double exponent) {
	if (exponent <= 0) {
...
LossBasedControlConfig::LossBasedControlConfig()
	...
	increase_offset(RtcDataRate::BitsPerSec(1000)),
	loss_bandwidth_balance_increase(RtcDataRate::KilobitsPerSec(0.5)),
	loss_bandwidth_balance_decrease(RtcDataRate::KilobitsPerSec(4)),
	loss_bandwidth_balance_reset(RtcDataRate::KilobitsPerSec(0.1)),
...
LossBasedBandwidthEstimation::LossBasedBandwidthEstimation()
	...
	average_loss_(0),
	average_loss_max_(0),
	loss_based_bitrate_(RtcDataRate::Zero()),
	acknowledged_bitrate_max_(RtcDataRate::Zero()),
...
void LossBasedBandwidthEstimation::UpdateAcknowledgedBitrate(
	RtcDataRate acknowledged_bitrate,
	Timestamp at_time) {
...
RtcDataRate LossBasedBandwidthEstimation::Update(Timestamp at_time,
	...
	if (loss_report_valid && config_.allow_resets &&
		...
		RtcDataRate new_increased_bitrate =
		...
		const RtcDataRate new_increased_bitrate_cap = BitrateFromLoss(
		...
		const RtcDataRate new_decreased_bitrate_floor = BitrateFromLoss(
			loss_estimate_for_decrease, config_.loss_bandwidth_balance_decrease,
			config_.loss_bandwidth_balance_exponent);
		RtcDataRate new_decreased_bitrate =
...
void LossBasedBandwidthEstimation::Initialize(RtcDataRate bitrate) {
...
RtcDataRate LossBasedBandwidthEstimation::decreased_bitrate() const {
